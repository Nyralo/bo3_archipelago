...
REGISTER_SYSTEM_EX( "zm_blockers", &__init__, &__main__, undefined )

#define STR_NAV_VOLUME_TRAVERSAL	"air_buy_gate"		// script_noteworthy on a traversal node.  Indicates it's used for nav volume 
...
function door_init()
{
	...
	targets = ArrayCombine( ent_targets, node_targets, false, true );
...
{
	if( isdefined( self.script_noteworthy ) && self.script_noteworthy == STR_NAV_VOLUME_TRAVERSAL )
	{
...
function door_buy()
{
	...
	who RecordMapEvent(ZM_MAP_EVENT_PURCHASE_DOOR, GetTime(), who.origin, level.round_number, cost);
...
function blocker_update_prompt_visibility()
{
	...
	//similar to the logic in _zm_perks.gsc to update prompt visibility
	dist = 128 * 128;
	while( true )
	{
		...
		if( isdefined( players ) )
		{
			for( i = 0; i < players.size; i++ )
			{
				...
				{
					if( IS_DRINKING( players[i].is_drinking ) )
					{
...
function door_opened(cost,quick_close)
{
	...
	if(isdefined(self.doors))
	{
		...
		foreach ( door in self.doors )
		{
			...
			if( isdefined( door.script_noteworthy ) && door.script_noteworthy == STR_NAV_VOLUME_TRAVERSAL )
			{
				continue;
			}
...
//	trigger SetCursorHint( "HINT_NOICON" ); 
	...
	while( 1 ) 
	{
		...
		while( 1 )
		{
			...
			if(player_fails_blocker_repair_trigger_preamble(player, players, trigger, false))
			{
				break;
			}
...
function increment_window_repaired( s_barrier )//self = player
{
